...
const App = () => {
  ...
  useEffect(() => {
    const initFirebase = async () => {
      ...
      try {
        // Fetch global variables for configuration
        ...
        // Set up auth listener to update state (The most reliable source of user status)
        unsubscribe = onAuthStateChanged(authInstance, (user) => {
          ...
          setFirebaseServices(prev => ({
            ...
            isAuthReady: true, // This confirms the auth process has completed
          }));

          setIsAuthenticated(!!user);
        });

        // Initial sign-in: Attempt with custom token first, then anonymously
        ...
        // Set a user-friendly error if initialization fails
        setError("Application failed to initialize Firebase. Check console for details.");
      } finally {
