  ...
  useEffect(() => {
    const initFirebase = async () => {
      try {
        ...
        // Set up auth listener to update state
        const unsubscribe = onAuthStateChanged(authInstance, (user) => {
          ...
          // Check if the user is signed in (either custom/anonymous or email/password)
          if (user) {
            // This check is mainly for the initial custom token sign-in
        ...
        setFirebaseServices(prev => ({ ...prev, isAuthReady: true }));
      }
    };
    initFirebase();
  }, []);
